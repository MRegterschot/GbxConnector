...
func SetupRoutes(r *mux.Router) {
	...
	r.Handle("/chat/{uuid:[0-9a-fA-F-]{36}}/config", moderatorOrAdmin(http.HandlerFunc(handlers.HandleGetChatConfig))).Methods("GET")
	r.Handle("/chat/{uuid:[0-9a-fA-F-]{36}}/config", adminOnly(http.HandlerFunc(handlers.HandleUpdateChatConfig))).Methods("PUT")

	r.Handle("/ws/map/{uuid:[0-9a-fA-F-]{36}}", moderatorOrAdmin(http.HandlerFunc(handlers.HandleMapConnection))).Methods("GET")
	r.Handle("/ws/players/{uuid:[0-9a-fA-F-]{36}}", moderatorOrAdmin(http.HandlerFunc(handlers.HandlePlayersConnection))).Methods("GET")
	r.Handle("/ws/live/{uuid:[0-9a-fA-F-]{36}}", moderatorOrAdmin(http.HandlerFunc(handlers.HandleLiveConnection))).Methods("GET")
}
